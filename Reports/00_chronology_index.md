...
| `orchestrator_v3.py` (production) | **Current production orchestrator** (~747 lines; re-count after edits) | Located in `services/fusion_api/`. All v2 features plus: `httpx.AsyncClient` global connection pool via `lifespan()`, FFmpeg RAM pipes (`pipe:0`/`pipe:1`), `BackgroundTasks` for async DB saves, bulk `delete().where()` (N+1 fix), `.env` environment variables (no hardcoded keys), pre-compiled regex at module scope with `re.compile()`, Groq `llama-3.3-70b-versatile` when `USE_PRODUCTION_MODEL=True`, optional Cerebras 8B when `False` for QA. Entrypoint: `uvicorn.run("orchestrator_v3:app", ...)`. |

---

### Phase 9 Production Services (Current — Final Form)

**Canonical tree (this repo):** `services/` — audio (`audio_api/`), vision (`image_video_api/`), text (`text_api/`), fusion (`fusion_api/`). IEEE-style write-up and figures: `Reports/main.tex`.
